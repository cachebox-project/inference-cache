...
# the reconciler should NOT render a Deployment/Service, status.endpoint
# should mirror spec.endpoint, observedGeneration should advance, Ready
# should be True with reason ExternalEndpointAccepted, and a matching engine
# pod should come out of admission with LMCACHE_REMOTE_URL pointing at the
# operator-supplied endpoint. Also exercises CacheBackend printer columns
# and the validating webhook's negative path.
