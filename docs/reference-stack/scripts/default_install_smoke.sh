...
log "creating typed PodLocal MP backends"
...
for sample in \
  ...
  config/samples/cache_v1alpha1_pdtopology.yaml \
...
kubectl -n "$SYSTEM_NAMESPACE" wait --for=condition=Available --timeout="$READY_TIMEOUT" \
  deployment/inference-cache-controller-manager deployment/inference-cache-server
kubectl -n "$SMOKE_NAMESPACE" get cachebackend host-only managed-redis >/dev/null

log "PASS: default install, control-plane APIs, server surfaces, samples, doctor, typed MP admission, managed Redis, and idempotent re-apply"
