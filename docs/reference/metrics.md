...
| `inferencecache_snapshot_auth_total` | `result` | One increment per `/snapshot` request reaching the auth middleware, labeled by outcome. | `result` ∈ `ok` (bearer validated, controller SA, handler invoked), `unauth` (missing/empty/malformed bearer, OR TokenReview said `Authenticated=false` with `Status.Error` empty — a routine bad token), `forbidden` (TokenReview authenticated a non-controller identity), `error` (TokenReview transport failure, nil response, OR TokenReview returned with `Status.Error` populated — the authenticator chain itself faulted: webhook timeout, parse error, downstream cert failure — all fail-closed 503). A non-trivial `unauth` rate against a steady controller poller cadence usually means the projected SA token / RBAC path is broken; a non-trivial `error` rate means the apiserver-side authenticator chain is having trouble and is the right alert surface for "investigate the apiserver, not the controller." |
