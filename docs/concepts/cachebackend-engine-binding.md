...
The `inferencecache.io/skip-inject` annotation is the explicit escape hatch: any non-empty value other than `false`/`0`/`no`/`off`/`disable`/`disabled` (case-insensitive) opts the pod out. Use it for pods you've already pre-wired or that should run vanilla for a debugging experiment.
