...
| Per-tenant | `status.tenants[]` — `{id, indexEntries, hitRate}` | Per-tenant footprint. `indexEntries` summed across all tenant rows equals the cluster total by construction (it is the per-tenant breakdown of `prefixes.summary.total`). The empty-string `id` is the untenanted bucket. **No per-tenant memory column** — engine memory is tenant-unaware (a tenant sharing an engine would double-count its total), so it is not honestly attributable. For memory, read the per-replica row below; see the enforcement boundary in [`cachetenant-identity-and-quota.md`](cachetenant-identity-and-quota.md). |
