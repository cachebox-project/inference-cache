...
`hitRate` (on both `status.replicas[]` and `status.tenants[]`) is **optional and absent until reported**: the key is omitted while the stats reporter has not emitted yet, and present once it has. That "absent = not yet reported" is deliberately distinct from a present `"0"` (a real 0% hit rate) — the same nil-means-unreported convention used by `CacheBackend.status.indexParticipation.hitRate`, so the two surfaces read consistently. `tenants[].indexEntries` is likewise a pointer (`*int64`) to mirror `CacheTenant.status.indexEntries` (nil = "not yet computed"), but the poller only runs after a successful snapshot scrape, so an emitted tenant row always carries a real count — a present `0` is a genuinely empty tenant, never a fabricated absence. The summary field `prefixes.summary.total` is a non-pointer, non-`omitempty` int and is always rendered (including `0`). Other numeric leaf fields carry `omitempty`, so an observed zero is dropped from the wire but is unambiguous (it is not a "not reported" sentinel — only `hitRate` distinguishes that via nil).
