...
- `status.replicas[]` is a map-list keyed on `(tenant, id)` so two engine pods sharing a `metadata.name` across namespaces (the same tenant id and pod name, but different namespaces — e.g. `ns-a/vllm-0` vs `ns-b/vllm-0`) are kept as separate rows. The `tenant` field is optional for backward compatibility with consumers that wrote replicas before it existed; the absent value is treated as empty for keying.
- The status is populated by the controller scraping the server's internal `/snapshot` endpoint. `/snapshot.replicas[]` carries the same `(tenant, replicaId)` identity plus per-replica `prefixCount` and `lastEventAt`, which the controller also projects into [`CacheBackend.status.indexParticipation`](cachebackend-api.md#index-participation).
