...
| `spec.minimumMatchedTokens` | integer | Minimum *matched* prefix token count required AFTER the index lookup runs for `PREFIX_MATCH` to surface. Replicas whose matched overlap falls below this floor are filtered; if none survive, the response is downgraded to `NO_HINT`. Defaults to `64` (4 KV blocks at the typical 16-token block size) so the server filters trivial 1-block chat-template overlaps even when no operator-side policy exists. Set to `0` to disable enforcement entirely (e.g. raw-recall benchmarking). Distinct from `minimumPrefixTokens` — that field is a request-side gate; this is a result-side floor. Minimum `0`. |
