...
- **Status is write-only-on-change.** Every writer gates on an equality check and skips the write when nothing changed, keeping `resourceVersion` stable and watch traffic quiet. The write *mechanism* depends on the writer count: the **multi-writer** statuses (`CacheBackend`, with two writers; `CacheTenant`) use field-scoped `Status().Patch` so concurrent writers never clobber each other's fields, while the **single-writer** `CacheIndex` singleton uses a full-object `Status().Update` (safe precisely because there is exactly one writer). The shared rule is "don't churn"; field-scoped `Patch` is how the contended statuses avoid clobbering, not a universal requirement.
...
Several statuses are written by more than one component. Coexistence works because each **contended** writer uses field-scoped `Status().Patch` (write-only-on-change), so it touches only its own fields and never clobbers a co-writer's. (The single-writer `CacheIndex` singleton is the exception noted above — it writes the full object via `Status().Update`.)
...
2. **Intentionally declarative.** The field scaffolds a not-yet-built controller; the type godoc says so explicitly *and* names the tracking work. The fields this state covers today are `CacheTenantCryptoSpec` (godoc'd "reserved for future cryptographic isolation"), `PromptTemplate.slots`, and `PDTopology`'s prefill/decode pools — each scaffolded ahead of its render / disaggregation controller. Tightening each of those godocs to *name* the tracking effort (not merely gesture at "future" work) is itself part of honoring this invariant.
