...
Several statuses are written by more than one component. Coexistence works because every writer uses `Status().Patch` write-only-on-change, scoped to its own fields:
...
2. **Intentionally declarative.** The field scaffolds a not-yet-built controller; the type godoc says so explicitly *and* names the tracking work (e.g. `PromptTemplate` slots wired by the render series, `PDTopology` pools for phase-2 disaggregation, `CacheTenantCryptoSpec` reserved for future cryptographic isolation).
