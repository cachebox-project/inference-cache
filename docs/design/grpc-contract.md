...
- **ReplicaStats** `{ string replica_id = 1; int64 cache_memory_bytes = 2; float hit_rate = 3; float pressure = 4; string client_version = 5; }`
  `client_version` is an opaque version string identifying the client-side cache library the reporting replica is linked against (e.g. an LMCache client release). It carries no semver semantics on the wire — the server stores it per replica and compares string-equality against its own probed backend (server-side) version to surface skew. Empty / unset means "unknown" and never triggers a skew condition by itself, preserving backward-compat for subscribers that don't fill it. The downstream wiring — subscriber populates the field, server exposes per-replica + backend versions on `/snapshot`, controller writes a `VersionCompatible` status condition on `CacheBackend` that extends the existing Ready gate — lands incrementally in follow-up PRs in the same change set. This closes the operator-visibility gap exposed when a deployment silently runs mismatched client/server cache-library versions and every offload write fails async with no signal.
