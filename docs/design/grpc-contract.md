...
- `LookupRouteRequest` gains `adapter_id` (11) and `PrefixEntry` gains `adapter_id` (5), with `CacheStateUpdate.adapter_id` (8) as the per-update default for entries that set none. The server's index key becomes **`(tenant_id, model_id, hash_scheme, adapter_id, prefix_hash)`** — `adapter_id` joins the existing partition ahead of the content hash.
