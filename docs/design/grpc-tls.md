...
`LookupRoute` payloads are normally metadata-only (prefix hashes + tenant/model IDs, KV-tensor-free; see `grpc-contract.md`). **Exception (dual-input):** a tokenizer-less gateway MAY send `prompt_text` (raw prompt text) for the server to tokenize, so `LookupRoute` can carry user prompt text on `:9090` — the server hashes it transiently and never stores it, but it is on the wire. That raises the port's data-sensitivity and **strengthens** the case for TLS: prefix-hash + tenant-ID leakage to cluster-level logging or packet capture was already a real exposure, and prompt text more so. TLS closes it without the client-cert distribution problem mTLS would impose on the gateway client.
