...
> * **Engine pods need host networking too — opt in with `spec.integration.engineHostNetwork: true`.** Mooncake's mesh is dialed *from* the engine, so an overlay engine pod cannot participate. With the flag set, the Pod webhook moves matched engine pods onto the host network (`hostNetwork` + `dnsPolicy: ClusterFirstWithHostNet`) alongside the usual `LMCACHE_*` wiring. Until it is set, admission **warns on every `type: Mooncake` apply** and the backend reports `Ready` while transferring **zero KV**.
>
...
Server-side (consumed by `ResolveCacheServer` when rendering the master pod):

| Key | Default | Purpose |
|---|---|---|
| `serverImage` | `docker.io/kvcacheai/mooncake:0.3.11.post1` *(pinned, non-floating; fully qualified)* | Container image for the standalone Mooncake master. Fully qualified (`docker.io/…`) so CRI-O nodes without short-name resolution configured don't reject it; it is the only version tag published on Docker Hub and is version-aligned with the `mooncake-transfer-engine` 0.3.11.post1 release on PyPI, so the master and the engine-side transfer-engine client are wire-compatible when the operator pins the engine's pip package to the same release. Pin to an `@sha256:` digest for non-local runs. (The master entrypoint and RPC/metadata/metrics port surface are confirmed against the real image on a live cluster; digest-pinning — ideally an `@sha256:` digest — remains the outstanding pre-release hardening. See the `TODO` on `defaultMooncakeMasterImage`.) |
| `serverCommand` | `mooncake_master --rpc_port=50051 --metrics_port=9003 --enable_http_metadata_server=true --http_metadata_server_host=0.0.0.0 --http_metadata_server_port=8080` | Master command line. The default launches the master with its RPC port (the `mooncakestore://` endpoint the engine dials), the Prometheus metrics port, and the **embedded HTTP metadata server** so the simplest deployment needs no external etcd/redis. Override to change the metadata backend, enable HA, etc. **Do not change the RPC (50051) or HTTP metadata (8080) ports through this override** — the rendered Service ports, the readiness probe, and `status.endpoint` are pinned to those values and are not parsed back out of this command string, so changing them here would leave the Service, probe, and engine wire pointing at dead ports (the backend would stay `Ready=False`). Synchronized port config is a deliberate v1alpha1 non-goal (the LMCache `serverCommand` carries the same constraint). |
...
Engine-side: the adapter injects the same `--kv-transfer-config '{"kv_connector":"LMCacheConnectorV1","kv_role":"<role>"}'` arg and the same `LMCACHE_*` / `VLLM_USE_V1` / `INFERENCECACHE_FAIL_OPEN` / `PYTHONHASHSEED` env as the LMCache adapter (the same `chunkSize` / `remoteSerde` / `localCPU` / `maxLocalCPU` `backendConfig` knobs apply), with `LMCACHE_REMOTE_URL=mooncakestore://<status.endpoint>`. The reserved args/env are therefore identical to the LMCache adapter's. The kvevent-subscriber sidecar is also identical (the KV-event stream comes from vLLM, not the L2 store; `--hash-scheme=vllm`, `--ignore-block-removed=true`).
...
| Field | Type | Purpose |
|---|---|---|
| `endpoint` | string | Observed endpoint clients should use. For External backends this is mirrored from `spec.endpoint`; for **Offload-managed** backends it is populated by the controller from the rendered Service. It stays **empty** for events-only backends (`spec.integration.mode=EventsOnly`), which provision no server and so have no address to publish. |
...
| Engine wire must accept the provider binding | The selected `(runtime, type)` adapter does not accept the selected provider protocol (`lm`, `resp`, or `mooncakestore`), including unsupported host-only bindings. |
...
| Runtime/backend pair must be supported by an installed adapter | Effective `(engine, spec.type)` pair has no registered runtime adapter, so the reconciler would fall back to unmanaged AND the pod webhook would fail-open without injecting engine config. The effective engine is resolved with the same helper the reconciler and pod webhook use: `spec.integration.engine` lower-cased, defaulting to `vllm` when unset. The shipping adapters target `vllm` (with LMCache, External, and Mooncake) and `sglang` (with LMCache and SGLangHiCache); the registry's `SupportedPairs` is the authoritative list. Applies to `spec.type=External` too: the External passthrough adapter has its own (vLLM-only) Supports gate, so `External` with `engine: sglang` is rejected at admission instead of silently un-wired downstream. Bypassed only for an empty `spec.type` (required-field rejection wins). The rejection message names both sides of the offending pair and lists the supported pairs the controller's registered adapters expose, e.g. `no runtime adapter supports the (engine="vllm", backend="AIBrix") pair; supported pairs in this build: vllm/External, vllm/LMCache, vllm/Mooncake, sglang/LMCache, sglang/SGLangHiCache`. |
...
| Injection | Resolves the runtime adapter via `runtime.Registry.Select(runtimeID, cache)` and calls `adapter.InjectEngineConfig(pod.Spec, endpoint, cache)`. The endpoint source is type-scoped: `External` uses trimmed `spec.endpoint` only (operator-authoritative; no fallback to potentially stale status), while server-backed managed types use `status.endpoint` only (built from the live Service; `spec.endpoint` is admission-rejected). Engine-local adapters publish no endpoint. An empty endpoint fails open without injection only when the selected adapter requires one; the optional `EndpointRequirement` capability lets SGLang HiCache declare that it does not, so its adapter is called with an empty endpoint and injects the native HiCache arguments. Events-only is a separate endpoint-free path: it skips `InjectEngineConfig` because it wires no KV connector and appends only the kvevent-subscriber sidecar. Adapters preserve existing user args/env and make repeat injection idempotent. |
