...
| `observedServerInstance` | string | `metadata.uid` of the cache-server pod the controller last observed `Ready` (Phase 1: the lex-smallest Ready pod owned by the managed Deployment). On a change from a non-empty prior value the reconciler cascade-restarts every engine Deployment that owns pods carrying this backend's `inferencecache.io/injected-by` (validated against the matching `injected-by-uid`), by patching `inferencecache.io/cache-server-restart-trigger` onto each Deployment's pod template — the same mechanism `kubectl rollout restart` uses. Rate-limited to once per ~30s per backend. Empty until the first Ready pod; an empty→set transition never cascades. Inert for External / unsupported-runtime backends. Operator-side recovery for the upstream LMCache `LMServerConnector` EPIPE-on-restart bug — see [LMCache/LMCache#3565](https://github.com/LMCache/LMCache/issues/3565). |
