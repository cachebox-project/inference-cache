...
| `matchedEnginePods` | integer | Snapshot count, at the last reconcile, of pods in the CacheBackend's namespace whose labels satisfy `spec.engineSelector`. Pointer in Go so nil ("not yet computed") is distinguishable from an observed `0` ("computed and zero pods match"). Refreshed at reconcile cadence — not a real-time per-pod counter. The per-pod signal is the K8s Events the mutating Pod webhook emits (see [Mutating Pod webhook](#mutating-pod-webhook-engine-wiring)). When `spec.engineSelector` is absent the field stays nil. |
...
`kubectl get cachebackend` displays the observed `status.endpoint` and a `Matched` column sourced from `status.matchedEnginePods`, so managed backends show both the endpoint and the operator-actionable engine-fleet count once reconciliation has populated them. An empty `Matched` cell means the count has not yet been computed (e.g. cold start before the first reconcile) or the CR has no `spec.engineSelector` configured.
...
- The mutating Pod webhook emits per-pod Events on the engine pod at admission time: `InjectedByCacheBackend` (Normal) on the success path naming the matched CR, and `NoMatchingCacheBackend` (Normal) when no CacheBackend in the pod's namespace claimed it. The no-match event is gated to namespaces with at least one CacheBackend whose `spec.engineSelector` is non-empty so unrelated pods in unrelated namespaces don't generate event-stream noise. Both events surface in `kubectl describe pod <engine-pod>`.
...
| Events | Emits a `Normal InjectedByCacheBackend` event on the engine pod after a successful injection (message names the matched `<namespace>/<name>`), and a `Normal NoMatchingCacheBackend` event when no CacheBackend in the pod's namespace claimed it. The no-match event is gated to namespaces that have at least one CacheBackend with a non-empty `spec.engineSelector` — otherwise unrelated pods across the cluster would generate event-stream noise. Deduped per pod CREATE via the `inferencecache.io/injected-by` annotation check (a re-admission of an already-injected pod produces no event) and skipped on dry-run admissions so the webhook honors its `sideEffects=None` contract. |
