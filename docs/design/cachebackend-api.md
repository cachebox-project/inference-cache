...
| `observedServerInstance` | string | Stable identifier for the current Ready cache-server pod set — comma-joined, lex-sorted by pod name, of every Ready pod's `metadata.uid` that is transitively controller-owned by the CacheBackend's Deployment. Typically a single UID (PVC-backed backends are single-replica; ephemeral ones usually are too); multi-replica ephemeral backends produce a comma-joined list so replacement of any replica advances the identifier and triggers the cascade. On a change from a non-empty prior value the reconciler cascade-restarts every engine Deployment that owns pods carrying this backend's `inferencecache.io/injected-by` + matching `injected-by-uid`, by patching `inferencecache.io/cache-server-restart-trigger` onto each Deployment's pod template — the same mechanism `kubectl rollout restart` uses. Rate-limited to once per ~30s per backend. Empty until the first Ready pod; an empty→set transition never cascades. Inert for External / unsupported-runtime backends. Operator-side recovery for the upstream LMCache `LMServerConnector` EPIPE-on-restart bug — see [LMCache/LMCache#3565](https://github.com/LMCache/LMCache/issues/3565). |
