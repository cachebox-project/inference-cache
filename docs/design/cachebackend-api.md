...
| Injection | Resolves the runtime adapter via `runtime.Registry.Select(runtimeID, cache)` and calls `adapter.InjectEngineConfig(pod.Spec, endpoint, cache)`, where `endpoint` is type-scoped: for `spec.type=External` the trimmed `spec.endpoint` (operator-authoritative; admission validates it; preferred over `status.endpoint` so a new pod admitting between a `spec.endpoint` update and the reconciler's mirror is wired to the fresh address, not the stale one), with `status.endpoint` as a defensive fallback. For managed types the source is `status.endpoint` only (the reconciler builds it from the live Service; `spec.endpoint` is admission-rejected on managed types). The adapter merges: existing args/env on the engine container are preserved; repeat injections are idempotent. |
