...
| `storage.pvc.storageClassName` | string | Optional StorageClass for the PVC. Immutable after creation — Kubernetes rejects StorageClass changes, so a later edit is ignored (the controller logs and keeps the existing class). Omit to use the cluster default StorageClass. See [Persistent storage](#persistent-storage). |
