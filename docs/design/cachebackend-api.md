...
| External backends cannot declare PVC storage | `spec.storage.pvc` set when `spec.type` is `External`. The controller provisions no workload for External backends, so there is nothing to mount a controller-provisioned PVC into; configure persistence on the pre-existing cache directly. |
