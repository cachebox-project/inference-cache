...
| `observedServerInstance` | string | Stable identifier for the current Ready cache-server pod set. Shape: `<pod-uid>:<restart-sum>` per Ready pod, comma-joined and lex-sorted by pod name; `<restart-sum>` is the sum of `pod.status.containerStatuses[].RestartCount` filtered to the cache-server's own containers (the container names declared on the owned Deployment's pod template). Sidecars injected by other admission webhooks (service-mesh proxies, Datadog, etc.) appear in `containerStatuses` but are absent from the template and are intentionally excluded — a sidecar crash-loop must not advance the identifier and roll the engine fleet. An in-place restart of a cache-server container (kubelet respawning a crashed container — OOM with `restartPolicy=Always` reuses pod.UID) DOES advance the identifier and is observable. Typically a single segment for PVC-backed / single-replica ephemeral backends; multi-replica ephemeral backends produce a comma-joined list. On a transition that reflects an actual replacement (a prior pod is gone, or a persisting pod's restart-sum advanced — NOT a rolling-update strict-superset midpoint) the reconciler cascade-restarts every engine Deployment that owns pods carrying this backend's `inferencecache.io/injected-by` + matching `injected-by-uid`, by patching `inferencecache.io/cache-server-restart-trigger` onto each Deployment's pod template — the same mechanism `kubectl rollout restart` uses. Rate-limited to once per ~30s per backend. Empty until the first Ready pod; empty→set never cascades. **Strict-superset midpoints are NOT persisted** — if they were, a failed rollout that gets rolled back (new pod briefly Ready, then killed, leaving the original pod alone) would later look like "the new pod was replaced" and false-cascade. The latch stays pinned to the prior baseline through superset transitions; the cascade fires only on a genuine replacement (an original pod drops or a persisting pod's restart-sum advances). **Stale-while-unavailable**: when no Ready cache-server pod exists at all (Deployment scaled to 0, mid-rollout, image-pull stuck), this field intentionally retains its prior value rather than clearing — clearing would turn the eventual recovery's `""` → `new-uid:0` transition back into a first-observation baseline and silently skip the cascade, defeating the controller's purpose. Inert for External / unsupported-runtime backends and cleared on transitions to those modes. Operator-side recovery for the upstream LMCache `LMServerConnector` EPIPE-on-restart bug — see [LMCache/LMCache#3565](https://github.com/LMCache/LMCache/issues/3565). |
