...
| Idempotency short-circuit | A pod whose **engine container** already carries `LMCACHE_REMOTE_URL` is left alone, so re-admissions don't churn JSON patches at the apiserver. The engine container is identified by the same canonical-name-or-single-container rule the vLLM adapter uses (so a sidecar that happens to read from the same remote cache does NOT suppress engine injection); the adapter itself is also idempotent at the merge level. |
