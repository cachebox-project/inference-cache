...
- **no event yet, within `firstEventTimeout`** → `Health=Pending`, `Ready=False` (`AwaitingFirstKVEvent`). The timeout clock starts when the managed Deployment first reports `Available`.
