...
type CacheBackendStatus struct {
	...
	// ObservedServerInstance is a stable identifier representing the
	// current set of Ready cache-server pods the controller observes
	// for this managed backend. The shape is `<pod-uid>:<restart-sum>`
	...
	// baseline; no engines exist yet) and a strict superset (the
	// rolling-update midpoint where old + new pods are both Ready
	// briefly — the cascade fires on the next transition that drops
	// the old pod). Strict-superset midpoints are NOT persisted as
	// the new baseline: if they were, a failed rollout that gets
	// rolled back (the new pod briefly Ready, then killed by failing
	// readiness, leaving the original pod alone) would later look
	// like "the new pod was replaced" and false-cascade, even though
	// the original cache-server process and existing engine sockets
	// never changed. Keeping the latch pinned through superset
	// transitions makes the rollback path a no-op. Trade-off: when
	// an operator legitimately scales the backend up, this field
	// lags the actual pod set until the next non-superset transition
	// re-anchors the latch — it is the cascade-decision audit trail,
	// not the operator's pod inventory (status.matchedEnginePods is).
