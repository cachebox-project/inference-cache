...
type CacheBackendSpec struct {
	...
	// below stamps a conservative 4Gi request / 8Gi memory limit on the
	// minimal-YAML path (when the field is OMITTED) so the cache server is
	// bounded by the cgroup rather than node-pressure OOM-killed by the
	// kubelet under heavy T2 write load — a cache-stress benchmark against
	// an un-limited lmcache-server repeatedly OOM-killed the pod within
	// minutes of T2 traffic, which the default limit eliminates. Operators
	// tune per-deployment by overriding the field; an explicit empty
	// `spec.resources: {}` is honored as suppression of the default
