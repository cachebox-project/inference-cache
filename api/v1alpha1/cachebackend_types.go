...
type CacheBackendStatus struct {
	...
	// RestartCount across the pod's containers, so an in-place
	// container restart (kubelet respawning a crashed cache-server
	// container — OOM with restartPolicy=Always reuses pod.UID)
	// advances the identifier and is observable. For most managed
	// backends this is a single segment; multi-replica ephemeral
	// backends (spec.replicas > 1 on non-PVC backends) produce a
	// comma-joined list.
	//
