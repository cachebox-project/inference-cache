...
type CacheBackendStatus struct {
	...
	// this write-once value keeps the elapsed window monotonic, so Degraded is
	// sticky. Written write-once and never cleared. A genuinely recreated
	// managed Deployment keeps the prior anchor; the gate re-evaluates from
	// it, which is safe because the engine event source is unchanged by a
	// cache-server restart. (Inert only on an Offload backend that has not yet
	// reported Available; on the EventsOnly path it is set on the first
	// reconcile.)
	// +optional
