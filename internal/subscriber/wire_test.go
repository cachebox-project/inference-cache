...
func TestStatsReporterPopulatesSnapshotReplicas(t *testing.T) {
	...
	const oneGiB = 1 << 30
	scraper := subscriber.NewMetricsScraper(metrics.Client(), subscriber.ScraperConfig{
		URL:                   metrics.URL,
		Tier:                  subscriber.CacheTierAuto,
		CacheSizeBytes:        oneGiB,
		MaxConcurrencyCeiling: 256,
	}, nil)
	reporter := subscriber.NewStatsReporter(icpb.NewInferenceCacheClient(conn), scraper,
		subscriber.Config{ReplicaID: "vllm-0", ModelID: "Qwen/Qwen2.5-0.5B-Instruct", TenantID: "tenant-a", HashScheme: "vllm"},
		subscriber.WithStatsInterval(20*time.Millisecond),
	)
