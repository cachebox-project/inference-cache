...
verify-naming: ## Fail if core-identity files reference OCI/Oracle (see CONTRIBUTING.md).
	@bad=$$(grep -rniEI '\boci\b|oci\.com|oraclecloud|\boracle\b' \
		api proto pkg/server/proto config/crd config/rbac config/default config/manager config/server config/webhook config/certmanager internal PROJECT go.mod 2>/dev/null || true); \
	if [ -n "$$bad" ]; then \
