...
- **On every push**, the `pre-push` hook runs `make ci` (naming + internal-refs + format + vet + golangci-lint + race tests + build) and blocks the push if anything fails. Reproduce it anytime with `make ci`. CI also runs heavier gates that are not part of the local push hook, including the Rust/network-backed `make tokenize-cgo-test` job for the optional `smgcgo` tokenizer build tag.
