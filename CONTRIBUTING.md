...
- **On every push**, the `pre-push` hook runs `make ci` (naming + internal-refs + format + vet + test + build) and blocks the push if anything fails. Reproduce it anytime with `make ci`.
