...
func main() {
	var (
		...
		adapterNames       = flag.String("lora-adapter-names", "", `comma-separated `+"`id=name`"+` map from the engine's internal LoRA id (BlockStored.lora_id, assigned in --lora-modules load order) to the stable adapter identity used as the index partition — the same string the gateway sends as LookupRouteRequest.adapter_id (e.g. "1=sql-lora,2=chat-lora"). Unmapped ids fall back to "lora:<id>". Omit ONLY for base-model / non-LoRA traffic (nil lora_id → the default "" partition). ANY LoRA adapter — even a single one — carries a non-nil lora_id that never maps to "", so the ingest identifier here MUST agree end-to-end with the gateway's LookupRoute query identifier or every lookup silently misses: either configure the mapping (both sides use the name) or deliberately use the "lora:<id>" fallback on both sides.`)
	)
	flag.Parse()
