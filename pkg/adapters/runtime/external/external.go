...
// their own persistence — spec.storage.pvc is rejected for External at
// admission). The C2 reconciler already short-circuits on type==External before
// even consulting an adapter (see CacheBackendReconciler.dispatch), so this
// method is a safety net for any future code path that goes through
// Registry.Select for an External CR — it must never accidentally provision a
// placeholder cache-server or PVC.
